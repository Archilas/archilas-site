...
export function HowItWorksStepper({
  ...
  return (
    <div
      ...
      className="flex flex-wrap justify-center gap-2 border-b border-border-dark"
    >
      {howSteps.map((item, index) => {
        ...
        return (
          ...
          >
            ...
            {item.label}
            {selected ? (
              <span aria-hidden className="demo-progress absolute inset-x-3 -bottom-px h-px bg-accent-dark" />
            ) : null}
          </button>
        );
      })}
