# Archilas Site — Design System

Mem0 rhythm: blue-wash hero with peeking demo tabs, Why Archilas band, cream how-it-works.

## Type

- Topbar: all-caps Geist, weight 450, tracking `0.16em`.
- H1: Geist 650 + Instrument Serif italic on **understands** only (+8–10% optical). Display ~10% smaller than the prior clamp.
- Sticky topbar: z-50, blur, isolation — always paints above demo/cards.
- Dual CTAs: 46px pills, radius 999. Join waitlist → + outline Contact.

## Rhythm

- Hero wash: `#E8F0F9 → #EEF4FA → #F5F8FB → #F7F7F5` plus large soft radials. Ink type. No cartoon clouds.
- Stack: chip→H1 20px, H1→reinforce 20px, reinforce→CTAs 28px, CTAs→demo 40px. Top pad 64px. Demo tabs peek above the fold at 1280.
- Demo: café Fridays story. Phone → ✓ Saved → bundle → smart answer. 15s, once in view, Replay.
- Why Archilas: three architecture points. No competitor names. MCP intended, in development.
- How it works: cream stepper + pills.
- Waitlist: cream, dual buttons.

## Color

| Token | Value | Role |
|------|-------|------|
| `--bg` | `#F6F3ED` | Cream ground |
| Hero wash | `#E8F0F9` → `#F7F7F5` | Soft sky, ink type |
| `--ink` | `#0A0A0A` | Type, Join waitlist |
| `--signal` | `#0F766E` | Play, focus |
| Gold | `#D4B46A` | Why-better index, bundle kinds |
